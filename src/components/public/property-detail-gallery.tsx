  ...
  return (
    <section id="galeri" className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Galeri Properti</h2>
          <p className="text-sm text-slate-500">Dokumentasi visual area bangunan dan fasilitas.</p>
        </div>
        <Badge className="rounded-full border border-blue-100 bg-blue-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-blue-600">
          ...
          <div key={category} className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {images.map((image) => (
                <div
                  key={image.id}
                  className="group relative h-64 overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-md"
                >
                  <Image
                    src={image.imageUrl}
