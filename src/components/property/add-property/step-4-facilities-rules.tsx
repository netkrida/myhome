...
export function Step4FacilitiesRules({ onDataChange, initialData }: Step4FacilitiesRulesProps) {
  ...
  const handleFacilityChange = (category: keyof Step4FormData, facility: string, checked: boolean) => {
    const currentFacilities = form.getValues(category) || [];
    if (checked) {
      form.setValue(category, [...currentFacilities, facility]);
    } else {
      form.setValue(category, currentFacilities.filter(f => f !== facility));
    }
  };
        ...
        <Tabs defaultValue="facilities" className="w-full">
          ...
          <TabsContent value="facilities" className="space-y-6">
            {facilityCategories.map((category) => {
              ...
              const selectedFacilities = form.watch(category.key) || [];
                      ...
                      {category.items.map((facility) => {
                        ...
                        return (
                          <div key={facility} className="flex items-center space-x-2">
                            <Checkbox
                              id={`${category.key}-${facility}`}
                              checked={isSelected}
                              onCheckedChange={(checked) => 
                                handleFacilityChange(category.key, facility, checked as boolean)
                              }
                            />
                            <label
                              htmlFor={`${category.key}-${facility}`}
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {PROPERTY_RULES.map((rule) => {
                    const isSelected = form.watch("rules")?.includes(rule) || false;
                    return (
                      <div key={rule} className="flex items-start space-x-2">
                        <Checkbox
                          ...
                          checked={isSelected}
                          onCheckedChange={(checked) => 
                            handleRuleChange(rule, checked as boolean)
                          }
                        />
                        <label
                          htmlFor={`rule-${rule}`}
